...
async def get_collection_by_id(collection_id: UUID, user: User = Depends(get_current_user), session: AsyncSession = Depends(get_session)):
...
async def delete_collection_by_id(collection_id: UUID, user: User = Depends(get_current_user), session: AsyncSession = Depends(get_session)):
    response = await session.execute(select(Collection).where(Collection.id == collection_id, Collection.user_id == user.id))
    collection = response.scalar_one_or_none()
    if not collection:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Collection not found")
